...
- 特に以下の３つの相反する要求の間で人がどのようにトレードオフしているかを調べる
    - ターゲットへの正確な到達
    - 運動エネルギー消費の少なさ
...
- タスク内容
  ...
  - 具体的な実験内容
    - 初期位置は半径 2.0 cm の円、ターゲットは半径 2.5 cm の円
    ...
    - 実験A
      - 力場による外乱がない環境での初期訓練の後、最初からforce field（式 1で与えられる環境）での実験
      - 被験者が初期の訓練時とは違った大きく曲がる軌道に気づき、その後運動戦略を適応させるために意識的な努力をするのではないか
      - 6人の被験者（男性5人、女性1人）
      - 5つのトライアルブロックでテスト、各ブロックの間に5分間の休憩
      - ブロック1では、力場のない環境で 70 回成功させる
      - ブロック2では、力場のない環境で 30 回成功した後、10 回ほど被験者には知らせずにforce fieldに切り替わる試行を含む 80 回の試行を実行
      - ブロック3とブロック4は、force fieldで 60 回の試行を成功させる
      - ブロック5は、force fieldで 30 回成功した後、被験者には知らせずに力場のない環境に切り替わる試行を 10 回ほど含む 80 回の試行を実行
    - 実験B
      - 意識的な適応の影響を最小限に抑えるため、力場の強さを試行ごとに徐々に増加させる
      - 合計10人の被験者（男性9人、女性1人）
      - 4つのトライアルブロックでテスト、各ブロックの間に5分間の休憩
      - ブロック1では、力場のない環境で 70 回成功させる
      - ブロック2では、力場のない環境 で 30 回成功した後、18回の試行で力場による外乱を試行ごとに直線的に上昇させ、force fieldで 40 回の試行を成功させる
      - ブロック3は、force fieldで 70 回の試行を成功させる
      - ブロック4は、force fieldで 40 回成功した後、被験者には知らせずに力場のない環境に切り替わる試行を 10 回ほど含む 80 回の試行を実行
...
- 図４の説明
  ...
  - Baseline（太い点線）は 適応前の力場がない環境での15試行の軌道平均
  - final force（太線）はforce fieldでの最後の15試行の軌道平均
  - before effectsは被験者には知らせずにforce fieldに切り替わる試行での10試行の軌道平均
  - after effectsは被験者には知らせずに力場のない環境に切り替わる試行での10試行の軌道平均
  - 実験Aの細い実線はブロック３での10試行ごとの軌道の平均
...
- 図５の説明
  ...
  - A：ブロック3における、forcefieldでの試行ごとの曲がり具合の平均
  - B:実験Bのブロック2における試行ごとの曲がり具合の平均（徐々に力場が増える試行を含む）
  - 灰色の破線は力場による外乱がゼロから徐々に上がっていることを示す
  - グラフのy軸は、試行の開始点からターゲットまでを結ぶ直線からの平均距離
  - ベースラインは破線
  - 太線は15試行分ごとの移動平均
- 図６の説明
  - ターゲットと目標を結ぶ線に垂直な方向（＝力場の外乱方向）に加わった力を、操作ハンドルに取り付けられた力センサで記録したもの
  - averaged:実験Aの場合の６人の平均
  - individual:実験Aの1人の被験者の個々の試行
  - Baselineは適応前のnull fieldでの15試行の平均
  - final forceはforce fieldでの最後の15試行の平均
  - before effectsは被験者には知らせずにforce fieldに切り替わる試行での10試行の平均
...
## モデル説明
 - SOC（ノイズがある中で、ある目的にとって最適な動きを計算するための数学的な枠組み）について
    ...
    - 式 12 の説明
      - 今回の場合の外力
      - $\beta$:フィールド強度を0から1にスケーリングするためのパラメータ
    - 式 13 の説明
      - 2次のコスト関数
...
- 人間の運動は、①「なるべくまっすぐ動きたい」②「正確にゴールしたい」③「筋力を節約したい」という3つの目標のバランスを取る戦略であると主張
